...
<template>
  <el-container class="main-container full-height">
    <el-header v-if="false" class="main-header">
      <div class="float-left main-title">
        <img src="../../assets/vform-logo.png" @click="openHome" />
        <span class="bold">{{ vfProductName }}</span> {{ vfProductTitle }}
...
<script>
import WidgetPanel from "./widget-panel/index";//左侧组件列表面板
import ToolbarPanel from "./toolbar-panel/index";//顶部按钮面板
import SettingPanel from "./setting-panel/index";//右侧属性设置面板
import VFormWidget from "./form-widget/index";//表单设计容器
...
export default {
  ...
  created() {
    this.vfProductName =
      (this.designerConfig && this.designerConfig.productName) ||
      (this.$vformInfo && this.$vformInfo.productName) ||
      "VForm Pro";
    this.vfProductTitle =
      (this.designerConfig && this.designerConfig.productTitle) ||
      (this.$vformInfo && this.$vformInfo.productTitle) ||
  ...
  methods: {
    ...
    //修改后的loadFieldListFromServer()
    loadFieldListFromServer() {
      if (!this.fieldListApi || this.fieldListApi.data.length == 0) {
        return;
      }
